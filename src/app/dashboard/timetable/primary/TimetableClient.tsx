...
const handleExportPNG = async (mode: 'FULL' | 'CLEAN' | 'DETAIL' = 'FULL') => {
  ...
  const handleRollover = useCallback(() => {
    ...
    rolloverWeek(weekNumber, weekNumber + 1, '2026-2027', branch, level)
      .then(res => {
        if (res.success) {
          router.push(`/dashboard/timetable/primary?week=${weekNumber + 1}`);
        } else {
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-4">
            <div className="flex gap-2">
              ...
              <button
                onClick={() => router.push(`/dashboard/timetable/primary?week=${weekNumber - 1}`)}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      ...
                      if (val && val > 0 && val !== weekNumber) {
                        router.push(`/dashboard/timetable/primary?week=${val}`);
                      }
                    }
                  }}
                  onBlur={(e) => {
            ...
            <div className="flex gap-2">
              <button
                onClick={() => router.push(`/dashboard/timetable/primary/detailed?week=${weekNumber}`)}
